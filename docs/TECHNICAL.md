...
## Before going live
- The portal logo is an HTML/CSS wordmark in Sora (`.wordmark` in `src/styles/app.css`, font in `--font-logo`). `public/logo.png` is the same logo as an image for other uses. The browser icon is `public/favicon.png`, and the app icons are `public/icon-*.png` and `public/apple-touch-icon.png`. Brand colours live in `src/styles/brand.css`.
