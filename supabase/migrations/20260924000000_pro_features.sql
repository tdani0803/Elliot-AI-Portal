...
-- Run this AFTER 20260923000000_init.sql. Safe to run once on an existing project.
--
-- Adds:
...
alter table public.calls
  ...
  add column lead_status       public.lead_status not null default 'new',
  add column won_value         numeric(12, 2) check (won_value >= 0),
...
alter table public.clients
  add column monthly_fee    numeric(10, 2) check (monthly_fee >= 0),
...
create index bookings_client_starts_idx on public.bookings (client_id, starts_at);
