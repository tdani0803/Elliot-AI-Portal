...
import { hasSupabaseEnv, keyFingerprint } from '../lib/rest.mjs';
  ...
  try {
    ...
    const reason = /app_secrets/.test(message) && /does not exist|not find/i.test(message)
      ...
      : /401|403|Invalid API key|JWT/i.test(message)
        ? `Supabase rejected the key: ${keyFingerprint()}. Compare it with the secret key in Supabase → Project Settings → API Keys.`
        : message.slice(0, 200);
